...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i]!=0)
        {
            arr_small[count2++] = arr_big[i];	//只取数组中不为零的数
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1) //寻找没有左右子树的节点 
        {
            length = findParent(i,huffman,count);   //得到各个叶结点的带权路径长度
            sum=sum+huffman[i][3]*length;/*自行填充代码*/  	//各个叶结点带权路径长度累加得到总长度WPL
        }
    }
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  //判断该节点是否有权值是否有父节点 
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  //找出有权值的且没有父节点的点 
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        ...
        if(huffman[i][3] < min1 && huffman[i][0] == -1&&i!=ii[0])  //找出有权值的且没有父节点的点 
        {
            min1 = huffman[i][3];
            ii[1] = i;
        }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    while(father != -1)
    {
        ...
        else if(huffman[father][2] == current)/*自行填充代码*/		//子结点是父结点的右子树，编码为1
             {code[start]='1';
			 }
